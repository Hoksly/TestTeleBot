...
def get_weather(lat:str, lon:str, exclude:str,  lan='eng'):
...
#gg = get_weather('50', '30', 'hourly, daily')


def detailed_48_hours(text):
    res = ''
    hour = datetime.now().hour
    for el in text:
        s = ''
        s += str(hour) + " o'clock: "
        s += str(int(el['temp'] - 272.15)) + ' °C, '
        s += el['weather'][0]['main'] + ', '
        s += str(el['pop']) + ' chance of rain'
        s += '\n'
        hour += 1
        if hour == 24:
            hour = 0
